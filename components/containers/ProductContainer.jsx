...
const PriceDisplay = ({ productData }) => {
  ...
  return (
    ...
    >
      <CustomText classes="text-sm text-primary font-bbold">
        {discountedPrice} M
      </CustomText>
      {hasDiscount && (
        <CustomText classes="text-sm text-gray-400 font-bbold line-through">
          {originalPrice}M
        </CustomText>
      )}
    </View>
...
const ProductContainer = ({ productData }) => {
  ...
  return (
    <Pressable
      ...
      className="border border-grey-600 rounded-xl flex flex-col h-fit"
    >
      <View className="relative">
        ...
        <Image
          ...
          contentFit="cover"
          className={`rounded-t-xl ${screenWidth < 500 ? "h-40" : "h-60"}`}
          transition={50}
      ...
      <View className="bg-dark rounded-b-xl px-[2px] py-1">
        <PriceDisplay productData={productData} />
        <View className="flex-row items-center justify-center content-center flex-wrap px-2 h-9">
          {productData?.ProductSizes?.filter((item) => item.quantity > 0).map(
            (item, index, filteredArray) => {
              ...
              return (
                <CustomText
                  key={index}
                  classes="text-xs text-center text-white font-bbold"
                  numberOfLines={2}
                >
                  {item.Size?.value}
                  {!isLast && ","}{" "}
